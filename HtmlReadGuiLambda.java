...
public class HtmlReadGuiLambda extends JPanel{
	...
	private JButton buttonDownload; // GUI download button declared
	private JTextField tf1,tf2; // Text fields for link and filename declared	
	
	public HtmlReadGuiLambda() { // Constructor to build the GUI
		setUp();
	}
	...
	private void setUp() {
		...
		add(buttonDownload);
		buttonDownload.addActionListener(e -> { // Lambda expression to perform the download button action
				try{
					URL url = new URL(tf1.getText()); // try statement to create URL with the text
					Download(url);
					
				}
				
				catch(IOException ioex) {// Catches input errors and prints in the console
					System.out.println("Input/Output Exceptions");
				}
				
				catch(IllegalArgumentException iaex) { // Catches illegal argument errors and prints in the console
	...
	public static void main(String[] args) {
		...
		HtmlReadGuiLambda htmlgui = new HtmlReadGuiLambda();
